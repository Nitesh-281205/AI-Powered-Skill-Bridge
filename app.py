...
def groq_json(prompt, system, max_tokens=4500, temperature=0.5):
    if not client:
...
def groq_text(prompt, system="You are SkillBridge, a career coach.", max_tokens=1600):
    if not client:
        return {"error": "API key missing."}
    try:
        response = client.chat.completions.create(
...
def groq_chat(messages, system, max_tokens=1200):
    if not client:
        return {"error": "API key missing."}
    try:
        response = client.chat.completions.create(
...
"""
    result = groq_json(prompt, "You are a precise career coach and talent analyst.", 5000, 0.7)
    if "error" in result:
        return result
...
Current question number: {question_index + 1} of 8"""
    ...
    result = groq_chat(messages, system, max_tokens=400)
    if "error" in result:
        return jsonify(result), 500
    raw = result["text"]
    try:
...
Write structured feedback covering overall impression, communication, technical knowledge, behavioural quality, top strengths, top improvements, and a final score out of 10."""
    result = groq_text(prompt, "You are an expert interview coach.", max_tokens=800)
    if "error" in result:
        return jsonify(result), 500
